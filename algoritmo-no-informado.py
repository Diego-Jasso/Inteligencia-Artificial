...
# Read Excel file
data = pd.read_excel('1.xlsx', dtype = {'Subtopic': int, 'Number': int, 'Duration': int, 'Value': int, 'Mandatory': int })
Kmin = 70
Kmax = 90
activities = []
...
for i in range(1, 9):
    ...
    sorted_subtopic = sorted(subtopic, key=lambda x: int(not x['mandatory']))
